...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder_holding_yolo_files", type=str, required=True)
    parser.add_argument("--yolo_class_list_file", type=str, required=True)

    args = parser.parse_args()

    folder_holding_yolo_files = args.folder_holding_yolo_files
    yolo_class_list_file = args.yolo_class_list_file
    ...
    if not os.path.exists(folder_holding_yolo_files+ os.sep+'VOC'):
         # If an XML folder does not already exist, make one
        os.mkdir(folder_holding_yolo_files+ os.sep+'VOC')

    os.chdir(folder_holding_yolo_files)

    if not os.path.exists(folder_holding_yolo_files+'/VOC'+os.sep +'IMAGE'):
    # If an XML folder does not already exist, make one
        os.mkdir(folder_holding_yolo_files+'/VOC'+os.sep +'IMAGE')
    

    if not os.path.exists(folder_holding_yolo_files+'/VOC'+os.sep +'XML'):
    # If an XML folder does not already exist, make one
        os.mkdir(folder_holding_yolo_files+'/VOC'+os.sep +'XML')
    

    xmlpath = folder_holding_yolo_files+'/VOC/XML' 
    imagepth = folder_holding_yolo_files+'/VOC/IMAGE' 
    ...
    for each_yolo_file in os.listdir(folder_holding_yolo_files):
        if each_yolo_file.endswith("txt"):
            ...
            if os.path.exists(each_yolo_file.replace('txt', 'jpeg')):
                ...
                target =  imagepth + os.sep +each_yolo_file.replace('txt', 'jpeg')
                shutil.copyfile(image_name, target)
            if os.path.exists(each_yolo_file.replace('txt', 'jpg')):
                image_name = each_yolo_file.replace('txt', 'jpg')
                target =  imagepth + os.sep +each_yolo_file.replace('txt', 'jpg')
                shutil.copyfile(image_name, target)
            if os.path.exists(each_yolo_file.replace('txt', 'png')):
                image_name = each_yolo_file.replace('txt', 'png')
                target = imagepth + os.sep +each_yolo_file.replace('txt', 'png')
                shutil.copyfile(image_name, target)
            ...
            # then we did NOT find an image that matches, and we will skip this code block
                ...
                # Start the XML file
                with open(xmlpath + os.sep + each_yolo_file.replace('txt', 'xml'), 'w') as f:
                    f.write('<annotation>\n')
